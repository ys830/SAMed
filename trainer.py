...
def trainer_camus(args, model, snapshot_path, multimask_output, low_res):
    from datasets.dataset_camus import CAMUS_dataset, RandomGenerator
    logging.basicConfig(filename=snapshot_path + "/log.txt", level=logging.INFO,
                        format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
    logging.info(str(args))
    ...
    batch_size = args.batch_size * args.n_gpu
    # max_iterations = args.max_iterations
    db_train = CAMUS_dataset(base_dir=args.root_path,split="train",start=args.train_start,end=args.train_end,skip=args.train_skip,
                               transform=transforms.Compose(
                                   [RandomGenerator(output_size=[args.img_size, args.img_size], low_res=[low_res, low_res])]))
    print("The length of train set is: {}".format(len(db_train)))
